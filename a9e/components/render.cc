    ...
    for (auto& e : scene.MakeEntityView<Transform,Render>()) {
        ...
        switch (render.render_type) {
            case RenderType_Char: {
                ...
                r->DrawChar(data.c, transform.pos.x, transform.pos.y, render.render_style);
                break;
            }
            case RenderType_Box: {
                RenderBox data = std::get<RenderBox>(render.data);
                r->DrawBox(data.c, transform.pos.x, transform.pos.y, data.w, data.h, render.render_style);
                break;
            }
            case RenderType_Bitmap: {
                ...
                for (auto& pixel : data.pixels)
                    r->DrawChar(pixel.c, transform.pos.x+pixel.x, transform.pos.y+pixel.y, render.render_style);
                break;
            }
            case RenderType_Text: {
                RenderText data = std::get<RenderText>(render.data);
                r->DrawText(data.text, transform.pos.x, transform.pos.y, render.render_style);
                break;
            }
            default: { }
        }
